# FCPXML Export Bug Fix - Complete Implementation Guide

## CONFIRMED: The Video Data

From your GoPro footage metadata:
- **File**: GX010331.MP4  
- **Resolution**: 3840 × 2160 (4K)
- **Frame Rate**: **119.88 fps** ← THIS IS THE PROBLEM
- **Video Format**: HEVC
- **Color Space**: ITU-R BT.709

## ROOT CAUSE

**FCPX does NOT support 119.88fps as a sequence format.**

FCPX can *import* and *play* 119.88fps footage, but it cannot *create* a 119.88fps timeline via FCPXML import. The timebase 120000 is not recognized by FCPX for sequence creation.

## FCPX-SUPPORTED SEQUENCE FRAME RATES

| Frame Rate | Timebase | Format Name | Status |
|------------|----------|-------------|--------|
| 23.976 fps | 24000 | FFVideoFormat4K2398 | ✓ Supported |
| 24 fps | 24 | FFVideoFormat4K24 | ✓ Supported |
| 25 fps | 25 | FFVideoFormat4K25 | ✓ Supported |
| 29.97 fps | 30000 | FFVideoFormat4K2997 | ✓ Supported |
| 30 fps | 30 | FFVideoFormat4K30 | ✓ Supported |
| 50 fps | 50 | FFVideoFormat4K50 | ✓ Supported |
| **59.94 fps** | **60000** | **FFVideoFormat4K5994** | ✓ **Supported - USE THIS** |
| 60 fps | 60 | FFVideoFormat4K60 | ✓ Supported |
| 119.88 fps | 120000 | N/A | ✗ **NOT SUPPORTED** |

---

## THE FIX

Convert the sequence to **59.94fps** (the closest supported frame rate to 119.88fps).

```xml
<!-- BEFORE (DOES NOT WORK) -->
<format id="r1" width="3840" height="2160" frameDuration="1001/120000s"/>

<!-- AFTER (WORKS) -->
<format id="r1" name="FFVideoFormat4K5994" width="3840" height="2160" frameDuration="1001/60000s"/>
```
...
## WHAT HAPPENS WITH 119.88fps FOOTAGE

When you have 119.88fps GoPro footage and export to a 59.94fps FCPXML sequence:

1. **Source assets** remain 119.88fps (FCPX reads the actual file)
2. **Timeline** plays at 59.94fps 
3. **Result**: Every 2nd frame is shown (normal speed) OR you can slow-mo to 50% speed for smooth playback
4. **FCPX handles** the frame rate conversion automatically
...
|-------|-------|----------|
| format="r1" rejected | 119.88fps NOT supported by FCPX | Use 59.94fps sequence format |
| GoPro high frame rate | 119.88fps source footage | Let FCPX convert during playback |

**The key change**: When `source_fps > 60.0`, automatically use `59.94` fps for the sequence format.
